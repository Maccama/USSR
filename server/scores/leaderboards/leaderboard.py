...
class GlobalLeaderboard:
    ...
    def has_scores(self) -> bool:
        """Returns a bool if leaderboard has any score."""
    ...
    def change_username(self, user_id: int, new_username: str) -> None:
        """Changes username in cache."""
        s = self._scores[user_id]
        s.username = new_username
        self._scores[user_id] = s

    # Made this a function to make inheritence easier.
    def _fetch_where_conds(self) -> tuple[tuple[str], tuple[Any]]:
    ...
    async def _fetch_scores(self) -> tuple[tuple[object, ...]]:
    ...
    async def __set_data_from_sql(self) -> None:
        ...
        scores_db = await self._fetch_scores()
        ...
        for idx, score in enumerate(scores_db):
            ...
            s = Score.from_lb_data(score, self.mode, self.c_mode, self.bmap)
            if idx + 1 < SIZE_LIMIT:
                self._scores[score[USER_ID_IDX]] = s

        # Store all user_ids
        self.users.extend(list(self._scores.keys()))
    ...
    def remove_user_score(self, user_id: int, banned: bool = False) -> None:
    ...
    def insert_user_score(self, s: "Score") -> None:
        ...
        # Now we append score and sort dict.
        self._scores[s.user_id] = s
        scores_sort = sorted(
            list(self._scores.values()),
            key=lambda _s: _s.pp if self.c_mode.uses_ppboard else _s.score,
        )
        self._scores = {s.user_id: s for s in scores_sort}
