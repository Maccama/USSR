...
async def perform_stats_update(uid_tup: tuple[int, int]):
    ...
    for mode in (Mode.STANDARD, Mode.TAIKO, Mode.MANIA, Mode.CATCH):
        for c_mode in (CustomModes.VANILLA, CustomModes.RELAX, CustomModes.AUTOPILOT):
            ...
            if not st:
                logger.error(f"Failed to load {mode!r} {c_mode!r} stats for {user_id}!")
                continue
...
async def recalc_chk(l: list[int]):
    ...
    logger.info("Chunk of recalculation completed!")


async def main():
    """The root of the server wide recalculator."""

    logger.info("Fetching a list of all users.")

    users_db = await services.sql.fetch_all("SELECT id, privileges FROM users")

    logger.info("Starting the stars recalculation of the whole server...")
